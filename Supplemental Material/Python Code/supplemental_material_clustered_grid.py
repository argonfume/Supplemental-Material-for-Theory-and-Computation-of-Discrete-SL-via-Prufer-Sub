#clustered grid regular shooting

import numpy as np
import matplotlib.pyplot as plt

#the grid
def clustered_grid(n, frac_pts, frac_dom):
    n1 = int(round(n*frac_pts))
    n2 = n - n1
    g1 = np.linspace(0, frac_dom*np.pi, n1+1)
    g2 = np.linspace(frac_dom*np.pi, np.pi, n2+1)
    return np.concatenate([g1, g2[1:]])

#the problem: y'' + lambda y = 0, y(0) = y(pi) = 0
...
grid = clustered_grid(n, frac_pts, frac_dom)
...
print(f"\nClustered grid: n={n}, {int(frac_pts*100)}% pts in first {int(frac_dom*100)}%")
...
ax.set_title(f"Residual sweep on clustered grid (n={n}, {int(frac_pts*100)}%/{int(frac_dom*100)}%) for {numeigs} eigenvalues")
ax.legend(); ax.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(f'Clustered_Residual_n{n}_eig{numeigs}.pdf', bbox_inches='tight')
plt.show()
...
plt.title(f"Eigenfunctions on clustered grid ({int(frac_pts*100)}% pts in first {int(frac_dom*100)}%, n={n}) for {numeigs} eigenvalues")
ax_curves.legend(loc='lower left', fontsize=9, ncol=4)
ax_curves.grid(alpha=0.3)
plt.tight_layout()
ax_curves.set_xticks(grid)
plt.savefig(f'Clustered_Eigenfunctions_n{n}_eig{numeigs}.pdf', bbox_inches='tight')
plt.show()

from google.colab import files
files.download(f'Clustered_Residual_n{n}_eig{numeigs}.pdf')
files.download(f'Clustered_Eigenfunctions_n{n}_eig{numeigs}.pdf')

#clustered grid prufer shooting
...
frac_pts = 0.7
frac_dom = 0.3
numeigs = 60 #number of desired eigen values
spacing = 100000
...
for n in [100]:
    ...
    exact = np.array([k*k for k in range(1, numeigs+1)])
    print(f"\nClustered grid (Prüfer): n={n}, {int(frac_pts*100)}% pts in first {int(frac_dom*100)}%")
    ...
    ax.set_title(f"Prüfer phase sweep on clustered grid (n={n}, {int(frac_pts*100)}%/{int(frac_dom*100)}%) for {numeigs} eigenvalues")
    ax.legend(); ax.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(f'Clustered_Prufer_Residual_n{n}_eig{numeigs}.pdf', bbox_inches='tight')
    plt.show()
    ...
    ax_curves.plot(np.pi, 0, 'ro', ms=7, label="target (π, 0)")
    ax_curves.set_xlabel('t'); ax_curves.set_ylabel('y (normalized)')
    plt.title(f"Eigenfunctions via Prüfer on clustered grid ({int(frac_pts*100)}% pts in first {int(frac_dom*100)}%, n={n}) for {numeigs} eigenvalues")
    ax_curves.legend(loc='lower left', fontsize=9, ncol=4)
    ax_curves.grid(alpha=0.3)
    plt.tight_layout()
    ax_curves.set_xticks(grid)
    plt.savefig(f'Clustered_Prufer_Eigenfunctions_n{n}_eig{numeigs}.pdf', bbox_inches='tight')
    plt.show()

    from google.colab import files
    files.download(f'Clustered_Prufer_Residual_n{n}_eig{numeigs}.pdf')
    files.download(f'Clustered_Prufer_Eigenfunctions_n{n}_eig{numeigs}.pdf')